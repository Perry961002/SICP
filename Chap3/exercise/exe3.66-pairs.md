...
```
    流的第一个序对是 (S1, T1)，其他部分是流 {(S1, T2), (S1, T3), ...} 和 Pa(S.r, T.r) 中的元素按交叉顺序出现的流。
(其中S.r表示流S中第一个元素之外的其他元素，用<a, b>表示流a, b中的元素按交叉顺序出现)，则有
                        Pa(S, T) = {(S1, T1), <{(S1, T2), (S1, T3), ...}，Pa(S.r, T.r)> }
```

- 用 F(i, j) 表示序对 (Si, Tj) 到序对 (S0, T0) 的距离，结合上面的描述，我们递归的给出这个函数表达式
```
    ...
    我们知道序对 (i-1, j-1) 到首序对的距离为 F(i-1, j-1)，结合Pa(S, T)的数学描述，明显的我们可以得到
                                        F(i ,j) = 2 * F(i-1, j-1) + 2
    特殊的，我们知道 F(1, 1) = 0，F(1, j) = 2 * j - 3。当 i = j时，有
                                        F(i ,j) = 2 * F(i-1, j-1) + 2
                                                .
                                                .
                                                .
    ...
    当 i < j 时，我们可以知道 i 一定会先被减到1，所以我们可以得到
                                        F(i ,j) = 2^(i-1)*F(1, j-i+1) + 2 + 2^2 + ... + 2^(i-2)
                                                = 2^i * (j-i) + 2^(i-1) - 2
```
